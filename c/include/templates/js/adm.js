...
$("input, select").change(function(){
 ...
 $(document).on('change', '#file_icon', function(){
  ...
  {
   alert("Invalid Image File! Only .png, .jpg, .jpeg"/*tpa=http://fork.ottv.tk/include/templates/js/Invalid Image File! Only .png, .jpg, .jpeg*/);
  }
...
function listch(na){
	...
	if(0){
		...
		for(var i=0;i < channels.length;i++) {
			...
			html+='<div title="'+channels[i]["playlist_url"]+channels[i]["stream_url"]+'" id="ch'+i+'" onmouseover="select='+i+';ch_over();" onclick="edit('+i+');" style="border: 3px solid red; height: 79px; width: 91px; position:relative; margin: 3px 4px; display: inline-block; overflow: hidden; text-align: center;"><img alt="'+channels[i]["logo_30x30"]+'" onerror="this.onerror=null;this.src=\'../images/nologo.png\'/*tpa=http://fork.ottv.tk/include/templates/images/nologo.png*/;" id="img'+i+'" src="'+channels[i]["logo_30x30"]+'" style="position:relative;height:75px;width:89px;padding:2px;"><div style="z-index: 0; position: relative; top: -35px; width: 85px; left: 3px; font-size: 14px; height: 34px; overflow: hidden; color: white; line-height: 0.9; padding-top: 3px; margin-left: 0px;">'+channels[i]["title"]+'</div></div>';
		}
		
		html+="</div><div id='ch"+i+"' onmouseover=\"select="+i+";ch_over();\" style='border: 3px solid red; height: 115px; width: 135px;position:relative; margin: 3px 4px; display: inline-block; overflow: hidden; text-align: center;' onclick=\"addch();\">Добавить новую ссылку!</div></center>";
	}
	else{
...
function renderCh(Ch,index,noadd){
	...
	else if(Ch["imgsrc"].indexOf("open.png"/*tpa=http://fork.ottv.tk/include/templates/js/open.png*/)>0||Ch["imgsrc"].indexOf("1px.png"/*tpa=http://fork.ottv.tk/include/templates/js/1px.png*/)>0||Ch["imgsrc"].indexOf("file.png"/*tpa=http://fork.ottv.tk/include/templates/js/file.png*/)>0||Ch["imgsrc"].indexOf("search.png"/*tpa=http://fork.ottv.tk/include/templates/js/search.png*/)>0) img='';
...
function edit(n){
	...
	$("#chtagid_description").htmlarea({
				...
				css: "jHtml/jHtmlArea.Editor.css"/*tpa=http://fork.ottv.tk/include/templates/js/jHtml/jHtmlArea.Editor.css*/,
                toolbar: [
...
function show_player(u,is_iptv,t){
	...
	else html='<object id="video_flash_api" class="vjs-tech" width="640" height="369" name="video_flash_api" data="GrindPlayer.swf"/*tpa=http://fork.ottv.tk/include/templates/js/GrindPlayer.swf*/ type="application/x-shockwave-flash" style="display: block;    background-color: #000;clear:both;">'+
...
function save_edit(){
	...
	if(chnumb!=n+1){
		var tmp=channels[n];
		channels[n]=channels[chnumb-1];
		channels[chnumb-1]=tmp;
	}
